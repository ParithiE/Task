...
export const getProcedureAssignments = async (procedureId) => {
    const url = `${api_url}/ProcedureUserAssignment?procedureId=${procedureId}`;
    const response = await fetch(url, {
        method: "GET",
    });
...
export const addProcedureAssignment = async (planId, procedureId, userId) => {
    const url = `${api_url}/ProcedureUserAssignment/AssignUserToProcedure`;
    var command = { planId: planId, procedureId: procedureId, userIds: userId };
    const response = await fetch(url, {
        method: "POST",
        headers: {
            Accept: "application/json",
